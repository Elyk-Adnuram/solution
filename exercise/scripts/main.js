  ...
  cards.forEach(function (card) {
    card.addEventListener("click", function () {
      ...
      if (selectedCards.length < 2) {
        ...
        selectedCards.push(card); //card is pushed to the selectedCards array
      }

      // If we have selected two cards, see if they match.
      if (selectedCards.length === 2) {
        var card1 = selectedCards[0];
        var card2 = selectedCards[1]; //card2 assigned to index 1 of selectedCards array
      ...
      if (matchedCards.length == cards.length) {
        // changed operator from > to == in order for alert to display
      ...
      // - #orange
      // - #red
      var deckElement = document.querySelector(".cards"); //changed spelling of class name to cards from Cards
      var deckColor = window.location.hash; // changed spelling of hashh to hash
      let newDeckColor = deckColor.slice(1); // remove the # from deckColor using slice method

      if (deckElement && newDeckColor) {
        // replace deckColor with newDeckColor
        var className = "cards--" + newDeckColor; // replace deckColor with newDeckColor
